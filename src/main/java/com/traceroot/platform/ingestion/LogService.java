...
public class LogService {
    ...
    public LogRecord createLog(LogEventRequest request) {
        ...
        boolean alreadyExists = incidentService.updateActiveIncidentIfExists(fingerPrint);
        if(!alreadyExists){
            ...
            if(matchList.size() >= INCIDENT_THRESHOLD){
                incidentService.createIncident(fingerPrint, matchList.size(), record.getCreatedAt());
            }
        }
        return record;
    }
